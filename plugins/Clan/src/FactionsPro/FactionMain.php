 ...
 * [X] Add faction description /g desc <faction>
...
class FactionMain extends PluginBase implements Listener {
    ...
    public function sendListOfTop10FactionsTo($s) {
        ...
        $row = array();
        $i = 0;
        $s->sendMessage($this->formatMessage("§b-§f Top 10 guilds KHỦNG nhất§b -", true));
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            $j = $i + 1;
            $cf = $resultArr['faction'];
            $pf = $this->getFactionPower($cf);
            $df = $this->getNumberOfPlayers($cf);
            $s->sendMessage(TextFormat::ITALIC . TextFormat::YELLOW . "$j §aBang " . TextFormat::GREEN . "$cf" . TextFormat::AQUA . " với " . TextFormat::RED . "$pf §fsức mạnh" . TextFormat::GREEN . " và " . TextFormat::LIGHT_PURPLE . "$df §fngười chơi" . TextFormat::RESET);
            $i = $i + 1;
        }
    }
   ...
   /* public function pointIsInPlot($x, $z, string $level) {
        ...
        $array = $result->fetchArray(SQLITE3_ASSOC);
        return !empty($array);
    }*/ //Under construction and maybe will fix in the future -Adam-

    public function cornerIsInPlot($x1, $z1, $x2, $z2, $level) {
